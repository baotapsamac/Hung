...
struct Preferences
{
  QString interfaceLanguage = "vi_VN"; // Empty value corresponds to system default
  QString helpLanguage; // Empty value corresponds to interface language
  QString displayStyle = "lingoes-blue"; // Empty value corresponds to the default one
...
#ifdef Q_OS_WIN
  QRect maximizedMainWindowGeometry=QRect(0,23,1366,705);//sửa
...
DEF_EX( exCantUseHomeDir, "Can't use home directory to store TĐVK preferences", exError )
DEF_EX( exCantUseIndexDir, "Can't use index directory to store TĐVK index files", exError )
...
/// /usr/share/apps/goldendict, under Windows C:/Program Files/TDNDict.
