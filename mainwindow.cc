...
//  historyChanged();
  ...
  setWindowTitle( "Từ điển chuyên ngành Vũ Khí - Trường Sỹ quan Kỹ thuật Quân sự" );
...
void MainWindow::addGlobalAction( QAction * action, const char * slot )
{
 ...
 // ui.searchPane->addAction(action);//thêm
...
void MainWindow::updateTrayIcon()
{
  ...
  if ( trayIcon )
  {
    ...
    trayIcon->setToolTip( "Từ điển chuyên ngành Vũ Khí - Trường Sỹ quan Kỹ thuật Quân sự" );
  }
  ...
  if ( view )
  {
    ...
    if( !str.isEmpty() )
    {
      ...
      if( !blockUpdateWindowTitle )
        setWindowTitle( tr( "%1 - %2" ).arg( str, "Từ điển chuyên ngành Vũ Khí - Trường Sỹ quan Kỹ thuật Quân sự" ) );
      blockUpdateWindowTitle = false;
    }
  }
}
...
void MainWindow::installHotKeys()
{
       ...
       cfg.preferences.enableClipboardHotkey )
  {
    ...
    {
      QMessageBox::critical( this, "TĐVK",
...
void MainWindow::latestReleaseReplyReady()
{
  ...
  if ( success && latestVersion > PROGRAM_VERSION && latestVersion != cfg.skippedRelease )
  {
    QMessageBox msg( QMessageBox::Information,
                     ...
                     tr( "Version <b>%1</b> of TDNDict is now available for download.<br>"
...
{
  QDesktopServices::openUrl( QUrl( "http://tdnu.edu.vn" ) );
}
    ...
    if( dictionaries[ x ]->getId() == id.toUtf8().data() )
    {
      if( dictionaries[ x ]->getDictionaryFilenames().size() > 0 )
      {
        ...
        if ( !QFileInfo( fileName ).isDir() )
        {
          ...
          // Z:\TDNDict\content\-=MDict=-\Test.mdx
