...
mac {
    ...
    QMAKE_POST_LINK = mkdir -p TDNDict.app/Contents/Frameworks & \
                      cp -nR $${PWD}/maclibs/lib/ TDNDict.app/Contents/Frameworks/ & \
                      mkdir -p TDNDict.app/Contents/MacOS/locale & \
                      cp -R locale/*.qm TDNDict.app/Contents/MacOS/locale/ & \
                      mkdir -p TDNDict.app/Contents/MacOS/help & \
                      cp -R $${PWD}/help/*.qch TDNDict.app/Contents/MacOS/help/

    CONFIG += zim_support
    !CONFIG( no_chinese_conversion_support ) {
        CONFIG += chinese_conversion_support
        CONFIG( x86 ) {
            QMAKE_POST_LINK += & mkdir -p TDNDict.app/Contents/MacOS/opencc & \
                                 cp -R $${PWD}/opencc/*.json TDNDict.app/Contents/MacOS/opencc/ & \
                                 cp -R $${PWD}/opencc/*.ocd TDNDict.app/Contents/MacOS/opencc/
        } else {
            QMAKE_POST_LINK += & mkdir -p TDNDict.app/Contents/MacOS/opencc & \
                                 cp -R $${PWD}/opencc/x64/*.json TDNDict.app/Contents/MacOS/opencc/ & \
                                 cp -R $${PWD}/opencc/x64/*.ocd TDNDict.app/Contents/MacOS/opencc/
        }
    }
}
