...
namespace
{
  QDir getHomeDir()
  {
    ...
    result = QDir::home();
    #ifdef Q_OS_WIN32
      if ( result.cd( "Application Data/TDNDict" ) )
        return result;
      char const * pathInHome = "TDNDict";
...
Class load() throw( exError )
{
  ...
  if ( !QFile::exists( configName ) )
  {
    ...
    Class c;
c.paths.push_back(Path("content",true));//THÊM
    #ifdef Q_OS_LINUX
...
//      c.soundDirs.push_back( SoundDir( "C:/Program Files/WyabdcRealPeopleTTS", "WyabdcRealPeopleTTS" ) );
    ...
    c.mediawikis = makeDefaultMediaWikis( false );//sửa
...
//c.paths.push_back(Path("C:\\Users\GMMX\Desktop\build-goldendict-Desktop_Qt_5_5_1_MinGW_32bit-Release\release\content",true));
  if ( !paths.isNull() )
  {
